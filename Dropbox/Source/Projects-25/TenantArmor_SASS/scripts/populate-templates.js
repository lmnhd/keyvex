  ...
  async processReasonTemplates(state, reason) {
    ...
    try {
      ...
      if (responseExists) {
        ...
        templates.push({
          promptId: `eviction_template_${state}_${reason}_response`,
          version: 'active',
      ...
      if (metaExists) {
        ...
        templates.push({
          promptId: `eviction_template_${state}_${reason}_meta`,
          version: 'active',
          templateType: 'meta',
          state,
          reason,
  ...
  async individualInsertItems(items) {
    for (const item of items) {
      try {
        ...
        await this.docClient.send(command);
        console.log(`      ✅ Inserted: ${item.promptId}`);
      } catch (error) {
        console.error(`      ❌ Failed to insert ${item.promptId}:`, error.message);
      }
    }
  }
