...
export async function processToolCreation(
  ...
  try {
    ...
    try {
      ...
      console.warn(`🏭 TRACE: AI call with primary model failed: ${error.message}. Falling back to default model.`);
      const fallbackModelConfig = getFallbackModel('toolCreation'); // Pass processName
      if (!fallbackModelConfig) {
        console.error("🏭 TRACE: CRITICAL - No fallback model configured for 'toolCreation'. Using hardcoded default.");
        modelInstance = createModelInstance('openai', 'gpt-4o'); // Hardcoded ultimate fallback
      } else {
      ...
      ({ object: parsedToolDefinition } = await generateObject({
        model: modelInstance, // This should now be correctly typed
        schema: productToolDefinitionSchema,
        prompt: userPrompt,
        system: systemPrompt,
      }));
